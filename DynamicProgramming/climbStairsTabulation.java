...
public class climbStairsTabulation {
    ...
    public static void main(String[] args) {
        int n = 3;
        int[] dp = new int[n+1];
        dp[0] = 1;   // here it is 1 because it has only one way if he is standing at the ground
        dp[1] = 1;   // here it is 1 because it has only one way if he is standing at 1st stair
        for(int i = 2; i <= n; i++){
            dp[i] = dp[i-1] + dp[i-2];
        }

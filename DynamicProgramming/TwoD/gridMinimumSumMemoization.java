...
public class gridMinimumSumMemoization {
    public static int minSum(int m, int n, int[][] nums, int[][] dp){
        if(m == 0 && n == 0){
            return nums[0][0];
        }
        if(m < 0 || n < 0){
        ...
        */
        ...
        int up =  minSum(m - 1, n, nums, dp);
        int left = minSum(m, n - 1, nums, dp);
        return dp[m][n] = nums[m][n] + Math.min(up , left);
    }
    public static void main(String[] args) {
        ...
        int res = minSum(m - 1, n - 1, nums, dp);
